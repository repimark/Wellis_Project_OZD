<?php
    ...
    if(!isset($_SESSION["u_id"])){
        ...
        while($row = $teruletQry->fetch_assoc()){
            $sql = "SELECT COUNT(k_id) AS db FROM `kilepett` WHERE t_id = '".$row["t_id"]."' AND YEAR(k_datum) = YEAR('$today') AND MONTH(k_datum) = MONTH('$today')";
            $qry = $conn->query($sql);
            $result = $qry->fetch_row();
            $sql1 = "SELECT COUNT(d_id) AS db FROM `dolgozok` WHERE t_id = '".$row["t_id"]."' AND YEAR(b_datum) = YEAR('$today') AND MONTH(b_datum) = MONTH('$today')";
