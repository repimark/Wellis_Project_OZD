...
include("connect.php");
$sql = "SELECT terulet.t_elnevezes AS 'Terület', terulet.t_id AS 'id', terulet_megjegyzes.tm_szoveg AS 'megj' FROM terulet , terulet_megjegyzes  WHERE terulet.t_id = terulet_megjegyzes.t_id";
		...
		<div class="card mg-2 text-center shadow rounded " style="background: white; border:;">
			...
			<div class="collapse" id="c<?php echo $row['id']; ?>">
				<div class="card card-body text-dark">
					...
					<p class="card-text">Igény : <?php echo $igenyMenny;?><br> </p>
					<!-- <p><?php echo $row["megj"]; ?></p> -->
					<input type="text" class="form-control" id="v<?php echo $row["id"]; ?>" placeholder="<?php echo $row['megj']; ?>"/>
					<button type="button" data-terulet='<?php echo $row["id"]; ?>' onClick="megjegyzMent(<?php echo $row['id']; ?>)" class="btn btn-info">Mentés</button>
				</div>
			</div>
		</div>
<?php
